...
//  * 1D and 2D Barcodes: CODE 39, ANSI MH10.8M-1983, USD-3, 3 of 9, CODE 93, USS-93, Standard 2 of 5, Interleaved 2 of 5, CODE 128 A/B/C, 2 and 5 Digits UPC-Based Extension, EAN 8, EAN 13, UPC-A, UPC-E, MSI, POSTNET, PLANET, RMS4CC (Royal Mail 4-state Customer Code), CBC (Customer Bar Code), KIX (Klant index - Customer index), Intelligent Mail Barcode, Onecode, USPS-B-3200, CODABAR, CODE 11, PHARMACODE, PHARMACODE TWO-TRACKS, Datamatrix, QR-Code, PDF417;
>>>>>>> Trial refactoring to lower case class names and Fooman namespace:src/Tcpdf.php
...
>>>>>>> Trial refactoring to lower case class names and Fooman namespace:src/Tcpdf.php
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		if (preg_match(Font\Data::$uni_RE_PATTERN_ARABIC, $s)) {
			$arabic = true;
		} else {
			$arabic = false;
		}
		// check if string contains RTL text
		if ($arabic OR ($this->tmprtl == 'R') OR preg_match(Font\Data::$uni_RE_PATTERN_RTL, $s)) {
			$rtlmode = true;
		} else {
			$rtlmode = false;
		}
...
>>>>>>> Trial refactoring to lower case class names and Fooman namespace:src/Tcpdf.php
	...
	protected function _puttruetypeunicode($font) {
		...
		$stream = Font\Data::$uni_identity_h;
		// ToUnicode Object
		$this->_newobj();
	...
	protected function _putcatalog() {
		...
		if ($this->pdfa_mode OR $this->force_srgb) {
			$iccobj = $this->_newobj();
			$icc = file_get_contents(sprintf("%s/sRGB.icc", __DIR__));
			$filter = '';
			if ($this->compress) {
				$filter = ' /Filter /FlateDecode';
				$icc = gzcompress($icc);
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if ($dom[$key]['tag']) {
				...
				// text
				$this->htmlvspace = 0;
				$isRTLString = preg_match(Font\Data::$uni_RE_PATTERN_RTL, $dom[$key]['value']) || preg_match(Font\Data::$uni_RE_PATTERN_ARABIC, $dom[$key]['value']);
